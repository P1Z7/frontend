...
const ArtistCard = ({ children, onClick, profileImage, isChecked = false, isSmall = false, isPost = false }: Props) => {
  return (
    <div className="flex-center h-fit w-fit flex-col gap-8 rounded-sm hover:cursor-pointer pc:gap-4" onClick={onClick}>
      <div className={classNames("relative h-88 w-88 pc:h-120 pc:w-120", { "pc:!h-80 pc:!w-80 ": isPost })}>
        <Image
      ...
      <p
        className={`pc:text-18 w-88 break-all text-center font-600 ${isSmall ? "h-20 text-14 text-gray-700" : "text-16 text-gray-900"} ${isPost && "pc:w-80 pc:!text-14 pc:!font-500"}`}
      >
        {children}
      </p>
    </div>
