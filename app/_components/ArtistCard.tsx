...
const ArtistCard = ({ children, onClick, profileImage, isChecked = false, isSmall = false }: Props) => {
  return (
    <div className="flex-center h-fit w-fit flex-col gap-8 rounded-sm hover:cursor-pointer pc:gap-4" onClick={onClick}>
      <div className={`relative h-88 w-88 ${isSmall ? "pc:h-80 pc:w-80" : "pc:h-120 pc:w-120"}`}>
        <Image
      ...
      <p className={`w-88 break-all text-center font-600 ${isSmall ? "h-20 text-14 text-gray-700 pc:w-80" : "text-16 text-gray-900 pc:text-18"}`}>{children}</p>
    </div>
  );
};
