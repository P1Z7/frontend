...
const FilterButton = ({ children, onClick, selected }: Props) => {
  return (
    <button
      onClick={onClick}
      className={`flex-center h-32 shrink-0 gap-4 rounded-full border border-gray-50 pl-12 pr-8 text-14 font-500 pc:text-16 ${selected ? "text-gray-700" : "text-gray-400"}`}
    >
      {children}
